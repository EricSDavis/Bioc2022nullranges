...
complex dependency structure, both in terms of their spatial pattern, 
and in terms of local correlation
of metadata (signal strength, cell-type-specificity, etc.) 
...
block bootstrap can do a much better job of estimating the variance of
the null distribution, compared to naive permutation of features.
...
move to a tile block across chromosome. 
...
command, and a `summarize` command.
Note that we need to use `tidyr::complete` in order to fill in
combinations of `iter` where the overlap was 0.
...
If one is interested in assessing the mean observed number of overlaps
per feature in `x`, instead of
of genome-wide statistics, one can also group by both
(`block`,`iter`). 10,000 total blocks may therefore be sufficient
to derive a bootstrap distribution, avoiding the need to generate many
bootstrap genomes of data.
