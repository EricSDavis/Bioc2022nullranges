...
example the sum observed number of overlaps for features in `x` in whole genome(or
...
mean observed number of overlaps per feature x is interested instead of genome-wide statistics, we can also do group by joint (`block`,`iter`) so that 10000 total blocks are enough to derive conclusion and don't need to run multiple times.
